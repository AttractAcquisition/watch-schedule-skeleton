            ...
            {(exportsQuery.data ?? []).map((e) => (
              <tr key={e.id} className="border-b border-border/60 last:border-0">
                ...
                <td className="px-5 py-2 font-mono text-[11px] text-muted-foreground">
                  {(e.generated_at ?? e.created_at).slice(0, 16).replace("T", " ")}
                </td>
                <td className="px-5 py-2">{TYPE_LABEL[e.export_type] ?? e.export_type}</td>
                <td className="px-5 py-2 max-w-[200px] truncate font-mono text-[11px] text-muted-foreground">
                  {e.file_name ?? "—"}
                </td>
                <td className="px-5 py-2 text-muted-foreground">{e.status}</td>
                <td className="px-5 py-2 text-right">
                  <button
                    className="text-xs text-muted-foreground underline-offset-4 hover:text-foreground hover:underline disabled:pointer-events-none disabled:opacity-40"
                    disabled={!e.file_url}
                    onClick={() => (e.file_url ? window.open(e.file_url, "_blank") : undefined)}
                  >
                    Download
                  </button>
                </td>
